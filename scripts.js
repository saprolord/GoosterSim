  ...
  while (battle) {
    ...
    if (dbstrike < (fighter2[2] - fighter1[3]) * 0.05) {
      ...
      if (hit > (fighter1[3] * 0.02-fighter2[2]*0.01)) {
        ...
        battlelog.push('<b>Hit!</b> <i>(Dodge throw: ' + Math.round(hit * 100, 0) + '% >' + fight1 + ' dodge x 2% - '+fight2+' speed x 1% =' + (fighter1[3] * 2 - fighter2[2] * 1) + '%)</i>');
        battlelog.push('<i>' + fight1 + ' takes ' + fighter2[1] + ' damages.</i> <b>' + fight1 + ' hp: ' + fighter1[0] + '<b>');
      } else {
        //hit missed / dodged
        battlelog.push('<b>Dodged!</b> <i>(Dodge throw: ' + Math.round(hit * 100, 0) + ' &lt ' + fight1 + ' dodge x 2% - ' + fight2 + ' speed x 1% =' + (fighter1[3] * 2 - fighter2[2] * 1) + '%)</i>');
      }
    }
    ...
    if (battle) {
      ...
      if (dbstrike < (fighter1[2] - fighter2[3]) * 0.05) {
        ...
        if (hit > (fighter2[3] * 0.02 - fighter1[2] * 0.01)) {
          fighter2[0] = fighter2[0] - fighter1[1];
          battlelog.push('<b>Hit!</b> <i> (Dodge throw: ' + Math.round(hit * 100, 0) + '% >' + fight2 + ' dodge x 2% - '+fight1+' speed x 1%=' + (fighter2[3] * 2 - fighter1[2] * 1) + '%)</i>');
          battlelog.push(fight2 + ' takes ' + fighter1[1] + ' damages. <b>' + fight2 + ' hp: ' + fighter2[0] + '</b>');
        } else {
          //hit missed / dodged
          battlelog.push('<b>Dodged!</b> <i> (Dodge throw: ' + Math.round(hit * 100, 0) + ' &lt ' + fight2 + ' dodge x 2% - ' + fight1 + ' speed x 1%='  + (fighter2[3] * 2 - fighter1[2] * 1) + '%)</i>');
        }
      }
      if (fighter2[0] < 1) {
        battle = false;
